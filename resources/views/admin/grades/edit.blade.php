                    ...
                    <form method="POST" action="{{ route("admin.grades.update", [$grade->id]) }}" enctype="multipart/form-data">
                            ...
                            <div class="form-group {{ $errors->has('company') ? 'has-error' : '' }}">
                                ...
                                <select class="form-control select2" name="company_id" id="company_id" required>
                                    {{-- @foreach($companies as $id => $entry) --}}
                                        <option value="{{ $companies->id }}" {{ (old('company_id') ? old('company_id') : $grade->company->id ?? '') == $companies->id ? 'selected' : '' }}>{{ $companies->comp_name }}</option>
                                    {{-- @endforeach --}}
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group {{ $errors->has('grade') ? 'has-error' : '' }}">
                                ...
                                <input class="form-control" type="text" name="grade" id="grade" value="{{ old('grade', $grade->grade) }}" required>
                                @if($errors->has('grade'))
