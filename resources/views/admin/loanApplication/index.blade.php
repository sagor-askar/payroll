...
<div class="content">
    ...
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
                ...
                <a class="buttonNew" href="{{ route('admin.loan.create') }}" style="color:white;">Add Loan Application</a>
                                    ...
                                    <tr data-entry-id="{{$value->id}}">
                                        ...
                                        <td>
                                            @can('loan_access_show')
                                                @if($value->status != 2)
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.loan.show', $value->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                                @endif
                                            @endcan
                                            @can('loan_access_edit')
                                                @if($value->status == 0)
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.loan.edit', $value->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                                @endif
                                            @endcan
                                            @if ($value->status != 1)
                                                @can('loan_access_delete')
                                                    <form action="{{ route('admin.loan.destroy', $value->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
