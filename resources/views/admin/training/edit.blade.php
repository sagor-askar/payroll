        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.training.update", [$training->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group {{ $errors->has('trainer_id') ? 'has-error' : '' }}">
                                    ...
                                    <select class="form-control select2" name="trainer_id" id="trainer_id" >
                                        ...
                                        @foreach($trainers as $id => $entry)
                                            <option value="{{ $entry->id }}" {{(old('trainer_id') ? old('trainer_id') : $training->trainer->id ?? '' ) == $entry->id ? 'selected' : '' }} >{{ $entry->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group {{ $errors->has('training_type_id	') ? 'has-error' : '' }}">
                                    ...
                                    <select class="form-control select2" name="training_type_id" id="training_type_id	" >
                                        ...
                                        @foreach($trainer_types as $id => $entry)
                                            <option  value="{{ $entry->id }}" {{(old('training_type_id') ? old('training_type_id') : $training->training_type->id ?? '' ) == $entry->id ? 'selected' : '' }}>{{ $entry->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group {{ $errors->has('training_skill_id	') ? 'has-error' : '' }}">
                                    ...
                                    <select class="form-control select2" name="training_skill_id" id="training_skill_id	" >
                                        ...
                                        @foreach($trainer_skills as $id => $entry)
                                            <option  value="{{ $entry->id }}" {{(old('training_skill_id') ? old('training_skill_id') : $training->trainingSkill->id ?? '' ) == $entry->id ? 'selected' : '' }} >{{ $entry->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group {{ $errors->has('cost') ? 'has-error' : '' }}">
                                    ...
                                    <input class="form-control" type="text" name="cost" id="cost" value="{{ old('cost' ,$training->cost) }}" required>
                                    @if($errors->has('cost'))
                            ...
                            <div class="col-md-6">
                                <div class="form-group {{ $errors->has('start_date') ? 'has-error' : '' }}">
                                    ...
                                    <input class="form-control date" type="text" name="start_date" id="start_date" value="{{ old('start_date',\Carbon\Carbon::parse($training->start_date)->format('d-m-Y')) }}" required>
                                ...
                                <div class="form-group {{ $errors->has('end_date') ? 'has-error' : '' }}">
                                    ...
                                    <input class="form-control date" type="text" name="end_date" id="end_date" value="{{ old('end_date',\Carbon\Carbon::parse($training->end_date)->format('d-m-Y')) }}" required>
                                ...
                                <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                                    ...
                                    <textarea class="form-control" name="description" id="description" cols="30" rows="2">{!! $training->description  !!}</textarea>
                        ...
                        @if($training->status == 1)
                            <div class="col-md-12">
                                <h4>Add / Terminate Employees Under This Training -</h4>
                            
                                @if($training->employee_id != null)

                                @php 
                                    $employee_id_list = json_decode($training->employee_id);
                                @endphp

                                <div class="form-group {{ $errors->has('employee') ? 'has-error' : '' }}">
                                    <label class="required" for="employee_id">Add Members</label>
                                    <select class="form-control select2 selectpicker" multiple data-live-search="true" name="employee_id[]" id="employee_id" required>
                        ...
                        @else
                            <h4 style="color: red"> To Add Employees, Please Approve the Training First!</h4>
                        @endif
