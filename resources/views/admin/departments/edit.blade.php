                        ...
                        <div class="form-group {{ $errors->has('sub_company') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="sub_company_id" id="sub_company_id">
                                @foreach($sub_companies as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('sub_company_id') ? old('sub_company_id') : $department->sub_company->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('branch') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="branch_id" id="branch_id">
                                @foreach($branches as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('branch_id') ? old('branch_id') : $department->branch->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
